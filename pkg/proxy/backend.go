...
import (
	...
	"git.100tal.com/wangxiao_jichujiagou_common/viktor-proxy/pkg/utils/math2"
	"git.100tal.com/wangxiao_jichujiagou_common/viktor-proxy/pkg/utils/sync2/atomic2"
)
...
func NewBackendConn(addr string, database int, config *Config) *BackendConn {
    ...
    directIp:="10.90.81.9"
    directPort := 8000
    conn, err := grpc.Dial(fmt.Sprintf("%s:%d", directIp, directPort), grpc.WithInsecure())
    if err != nil {
...
func (bc *BackendConn) newBackendReader(round int, config *Config) (*redis.Conn, chan<- *Request, error) {
    log.Warnf("newBackendReader ######################## %s", bc.addr)
    ...
    //go bc.loopReader(tasks, bc.conn, round)
...
//func (bc *BackendConn) loopReader(tasks <-chan *Request, c *grpc.ClientConn, round int) (err error) {
	defer func() {
		c.Close()
		for r := range tasks {
	...
	for r := range tasks {
		resp, err := c.Decode()
        //这里要把grpc的结果搞成redis协议的
        log.Warnf("BackendConn loopReader %s", resp.Value)
		if err != nil {
...
func (bc *BackendConn) loopWriter(round int) (err error) {
	...
	for r := range bc.input {
        ...
        for _, val := range r.Multi {
            log.Warnf("getHashKey %s", string(val.Value))
        }
...
//        cmd := new(kv.KvHash)
//        groupId := 1
//        res, err := cmd.KvHSet(&bc.conn, reqKey, reqField, reqValue, groupId);
//        if err != nil {}
//		kv := new(forwardKv)
//		res, err:=kv.HSet(r, &bc.conn)
//		if err != nil {
//		}
//        log.Warnf("cmd.KvHSet %d", res)
		kv := new(forwardKv)
		err := kv.Do(r, &bc.conn)
		log.Warnf("aaaa %+v", r.Resp)
		if err != nil {
			r.Resp = redis.NewErrorf("ERR handle response, %s", err.Error())
		}
		//resp := redis.NewInt([]byte(strconv.Itoa(1)))
		bc.setResponse(r, r.Resp, nil)
