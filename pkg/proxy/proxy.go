...
import (
	...
	"git.100tal.com/wangxiao_jichujiagou_common/viktor-proxy/pkg/models"
	"git.100tal.com/wangxiao_jichujiagou_common/viktor-proxy/pkg/utils"
	...
	"git.100tal.com/wangxiao_jichujiagou_common/viktor-proxy/pkg/utils/redis"
	"git.100tal.com/wangxiao_jichujiagou_common/viktor-proxy/pkg/utils/rpc"
	"git.100tal.com/wangxiao_jichujiagou_common/viktor-proxy/pkg/utils/unsafe2"
)
...
func (s *Proxy) FillSlots(slots []*models.Slot) error {
	...
	backendSlot := make(map[int]string)

	backend := s.config.BackendAddr
	for _, val := range backend {
		tmp := strings.Split(val, "#")
		slotId, err := strconv.Atoi(tmp[0])
		if err != nil {
			return err
		}
		backendSlot[slotId] = tmp[1]
	}

	for _, m := range slots {
		if m != nil {
			backend, ok := backendSlot[m.Id]
			if ok {
				m.BackendAddr = backend
