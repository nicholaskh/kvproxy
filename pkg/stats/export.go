...
func RegisterPushBackend(name string, backend PushBackend, emitPeriod time.Duration) {
	...
	if _, ok := pushBackends[name]; ok {
		log.Errorf(fmt.Sprintf("PushBackend %s already exists; can't register the same name multiple times", name))
	}
...
func emitToBackend(name string, emitPeriod time.Duration) {
	...
	for range ticker.C {
		...
		if !ok {
			log.Errorf(fmt.Sprintf("No PushBackend registered with name %s", name))
			return
		}
		err := backend.PushAll()
		if err != nil {
